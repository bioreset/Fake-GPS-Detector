...
class FirstScreenFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        repo_result = context?.let { LocationFromApiResponseRepository.getInstance(it) }!!
    ...
    private fun getStatus(result: String?) {
        if (result != null) {
            when (result) {
                "location" -> {
                    result_title.visibility = View.VISIBLE
                    result_title.text = showResult().status
                    result_lat.visibility = View.VISIBLE
                    result_lat.text = showResult().lat.toString()
                    result_long.visibility = View.VISIBLE
                    result_long.text = showResult().long.toString()
                    result_accuracy.visibility = View.VISIBLE
